...
# Calculate simulated displacement magnitude
displacement_magnitude = calculate_displacement_simulated()

# Function for plotting frequency domain comparison might look like this:
import numpy as np
import matplotlib.pyplot as plt
...
def plot_frequency_domain_comparison(freq, amp_sim_x, amp_real_x, amp_sim_y, amp_real_y):
    plt.figure(figsize=(14, 6))
    # X方向
    plt.subplot(1, 2, 1)
    plt.plot(freq[:N // 2], amp_sim_x[:N // 2], label='Simulated X')
    ...
    # Y方向
    plt.subplot(1, 2, 2)
    plt.plot(freq[:N // 2], amp_sim_y[:N // 2], label='Simulated Y')
...
if 0 in tracks:
    positions = np.array(tracks[0])
    xs_real, ys_real = positions[:, 0], positions[:, 1]
    fft_xs_real, fft_ys_real = fft(xs_real - np.mean(xs_real)), fft(ys_real - np.mean(ys_real))
    amp_x_real, amp_y_real = np.abs(fft_xs_real) / len(xs_real) * 2, np.abs(fft_ys_real) / len(ys_real) * 2

    frequencies_x, frequencies_y = fft(displacements_x), fft(displacements_y)
    freq = np.fft.fftfreq(N, 1 / fs)
    amp_x_sim, amp_y_sim = np.abs(frequencies_x) / N * 2, np.abs(frequencies_y) / N * 2
    plot_frequency_domain_comparison(freq, amp_x_sim, amp_x_real, amp_y_sim, amp_y_real)

    from sklearn.metrics import mean_squared_error, mean_absolute_error
    from math import sqrt

    # 确保 amp_x_sim, amp_x_real 等数组长度达到最小长度的数组的长度
    # 这是因为计算 MSE, RMSE 和 MAE 需要两个数组有相同长度
    min_length = min(len(amp_x_sim), len(amp_x_real))

    # 计算均方误差 (MSE), 均方根误差 (RMSE) 和平均绝对误差 (MAE)
    mse_x = mean_squared_error(amp_x_real[:min_length], amp_x_sim[:min_length])
    rmse_x = sqrt(mse_x)
    mae_x = mean_absolute_error(amp_x_real[:min_length], amp_x_sim[:min_length])

    print(f'X方向 - MSE: {mse_x}, RMSE: {rmse_x}, MAE: {mae_x}')

    # 对于Y方向也做相同的计算
    min_length = min(len(amp_y_sim), len(amp_y_real))
    mse_y = mean_squared_error(amp_y_real[:min_length], amp_y_sim[:min_length])
    rmse_y = sqrt(mse_y)
    mae_y = mean_absolute_error(amp_y_real[:min_length], amp_y_sim[:min_length])

    print(f'Y方向 - MSE: {mse_y}, RMSE: {rmse_y}, MAE: {mae_y}')
else:
    print("Track ID 0 not found.")
