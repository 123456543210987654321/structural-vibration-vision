...
def calculate_velocity(tracks, frame_rate):
    velocities = {}
    for track_id, points in tracks.items():
        ...
        velocities[track_id] = track_velocity
    return velocities

def analyze_time_domain(velocities, track_id, frame_rate):
    time_seconds = np.arange(len(velocities)) / frame_rate
    plt.figure(figsize=(10, 4))
    plt.plot(time_seconds, velocities, label=f"Track {track_id}")
...
velocities = calculate_velocity(tracks, frame_rate=120)  # 假设frame_rate已定义为视频的帧率
track_id = 0  # 这里仅示范一个轨迹，可以根据需要展示其他轨迹
if track_id in velocities:
    analyze_time_domain(velocities[track_id], track_id, frame_rate=120)
else:
...
for track_id, track in tracks.items():
    ...
    analyze_time_domain(velocities, track_id, frame_rate=120)
