...
def build_laplacian_pyramid(src,levels=3):
    ...
    for i in range(levels, 0, -1):
        GE = cv2.pyrUp(gaussianPyramid[i], dstsize=gaussianPyramid[i - 1].shape[1::-1])  # 注意 dstsize 参数
...
def reconstract_from_tensorlist(filter_tensor_list, levels=3):
    ...
    for i in range(filter_tensor_list[0].shape[0]):
        ...
        for n in range(levels - 1):
            # 确保在上采样前目标尺寸与下一图层相匹配
            target_shape = filter_tensor_list[n + 1][i].shape[1::-1]  # (width, height)
            up = cv2.pyrUp(up, dstsize=target_shape)  # 指定上采样的目标尺寸
            try:
                up += filter_tensor_list[n + 1][i]
            except ValueError as e:
                # 如果尽管指定了尺寸，但仍然存在尺寸不一致错误，这里可以捕捉错误进行调试
                print(f"Error adding up sampled layer with size {up.shape} to next layer with size {filter_tensor_list[n + 1][i].shape}")
                raise e
        final[i] = up
    return final
...
if __name__=="__main__":
    ...
    magnify_motion("test_video/test.avi",0.4,3)
